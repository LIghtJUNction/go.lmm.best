...
function isOnBoard(board: Board, point: Point): boolean {
  return point.y >= 0 && point.x >= 0 && point.y < board.length && point.x < board.length
}
...
export function applyMove(board: Board, point: Point, stone: Stone, seenPositions?: ReadonlySet<string>): MoveResult {
