...
function App() {
  ...
  const handleHumanMove = useCallback((point: Point) => {
    ...
    <div className="app-shell">
      ...
      <footer className="site-footer">
        ...
        <span className="footer-domain">
          go.lmm.best <i aria-hidden="true">·</i> WebMCP
        </span>
      </footer>
    </div>
  );
    ...
    <header className="topbar">
      ...
      >
        ...
        <span className="brand-wordmark">
          go<span>.</span>lmm<span>.</span>best
        </span>
      </button>
    ...
    <section className="lobby-layout" aria-labelledby="lobby-title">
      <div className="hero-column">
        ...
        <div className="hero-footnote">
          <span
            className={`mini-status ${webmcpStatus === "available" ? "is-live" : ""}`}
          >
            <span className="status-dot" />
            {webmcpStatus === "available" ? t.webmcpReady : t.demoHint}
          </span>
        </div>
        ...
        <div className="feature-row" aria-label={t.lobbyStatus}>
          <div className="feature-item">
            ...
            <span>
              <strong>{t.lobbyStatus}</strong>
              <small>{t.lobbyStatusDetail}</small>
            </span>
          </div>
          <div className="feature-item">
            <span className="feature-icon">
          ...
          <div className="feature-item">
            ...
            <span>
              <strong>{t.liveTools}</strong>
              <small>{t.copyToolHint}</small>
            </span>
          </div>
        </div>
      ...
      <div className="lobby-visual-column">
        ...
        <div className="signal-card">
          <div className="signal-card__top">
            <span className="signal-label">{t.statusLabel}</span>
  ...
  return (
    ...
    >
      <div className="section-heading">
        <h2 id="steps-title">{t.matchStepsTitle}</h2>
      </div>
  ...
  return (
    <div className="preview-wrap">
      <div className="preview-label">
        ...
        <span>{t.boardPreviewHint}</span>
      </div>
      <div className="board-card board-card--preview">
        <div className="board-card__top">
          <span className="small-caps">GO / 009</span>
          <span className="board-card__signal">
            <span className="status-dot is-live" />
            {t.live}
          </span>
        </div>
        ...
        <div className="board-card__bottom">
          <span>
            <i className="stone-key stone-key--black" />
            {t.human}
          </span>
          <span className="vs-label">VS</span>
          <span>
            <i className="stone-key stone-key--white" />
            {t.ai}
          </span>
        </div>
      </div>
...
function Searching({
  ...
  return (
    <section
      className="searching-layout"
      aria-labelledby="searching-title"
      data-state="searching"
    >
      <div className="searching-copy">
        <div className="searching-orbit" aria-hidden="true">
          <span />
          <span />
          <span />
          <div className="orbit-center">GO</div>
        </div>
        ...
        <div className="queue-meta">
          <div>
            ...
            <strong>{t.queuePositionValue}</strong>
          </div>
          <div>
            <span>{t.elapsed}</span>
      ...
      <aside className="waiting-panel">
        <div className="waiting-panel__head">
          <span className="panel-kicker">{t.statusLabel}</span>
          <span className="waiting-chip">
            <span className="status-dot is-pulsing" />
            {t.waiting}
          </span>
        </div>
        <div className="waiting-panel__main">
          <div className="waiting-glyph">
            <span />
            <span />
            <span />
          </div>
          <strong>{t.waitingFor}</strong>
          <p>{t.statusWaiting}</p>
        </div>
        <div className="tool-call-card">
          <span className="tool-call-card__label">{t.aiCallHint}</span>
          <ol className="agent-join-steps">
            <li>{t.agentJoinStepOne}</li>
            <li>{t.agentJoinStepTwo}</li>
            <li>{t.agentJoinStepThree}</li>
          </ol>
          <code>{JOIN_TOOL_EXAMPLE}</code>
          <span
            className={`tool-state ${webmcpStatus === "available" ? "is-live" : "is-muted"}`}
          >
            <span className="status-dot" />
            {webmcpStatus === "available" ? t.listening : t.webmcpUnsupported}
          </span>
        </div>
        <div className="waiting-panel__foot">
          <span className="waiting-pulse" />
          <span>
            {webmcpStatus === "available" ? t.webmcpStandby : t.demoHint}
          </span>
        </div>
      </aside>
    </section>
...
function GameRoom({
  ...
  return (
    <section
      ...
      data-state={isFinished ? "finished" : "playing"}
    >
      <div className="game-main-column">
        <div className="game-heading">
          <div>
            <h1 id="game-title">{t.gameTitle}</h1>
            <p className="game-subtitle">{t.gameSubtitle}</p>
          </div>
          <span className={`live-badge ${isFinished ? "is-finished" : ""}`}>
            <span className="status-dot" />
            {isFinished ? t.finishedTitle : modeText}
          </span>
        </div>
        <div className="game-board-card">
          <div className="game-board-card__top">
            <span className="small-caps">ROOM / 01</span>
            <span className="board-turn-label">
              <span className={`turn-stone turn-stone--${game.turn}`} />
              {isFinished
                ? t.finishedTitle
          ...
          <Board
            ...
            t={t}
          />
          <div className="game-board-card__bottom">
            ...
            <span className="board-size">9 × 9</span>
          </div>
        </div>
        <div
          className={`turn-banner ${isFinished ? "is-finished" : isHumanTurn ? "is-human" : "is-ai"}`}
        >
          <span className="turn-banner__indicator" />
          <span>{statusText}</span>
          {!isFinished && (
            <span className="turn-banner__right">
              {t.moves} {game.moves.length.toString().padStart(2, "0")}
            </span>
          )}
        </div>
      </div>
      ...
      <aside className="game-sidebar">
        ...
        <div className="stat-strip">
          <div>
            <span>{t.moves}</span>
            <strong>{game.moves.length.toString().padStart(2, "0")}</strong>
          </div>
          <div>
            <span>{t.captures}</span>
            <strong>
              {game.captures[game.humanColor].toString().padStart(2, "0")}
            </strong>
          </div>
          <div>
            <span>{t.lastMove}</span>
        ...
        {isFinished ? (
          ...
          <div className="game-actions">
            ...
            <button type="button" className="danger-button" onClick={onResign}>
              {t.resign}
              <Icon name="resign" />
            </button>
          </div>
  ...
  return (
    <div className="players-panel">
      <div className="players-panel__head">
        <span className="panel-kicker">{t.gameLive}</span>
        <span className="players-count">
          {isFinished ? t.finishedTitle : "01 / 01"}
        </span>
      </div>
      <div
        className={`player-row ${!isFinished && game.turn === game.humanColor ? "is-active" : ""}`}
      >
        <span className="player-stone player-stone--black" />
        <span className="player-copy">
          <strong>{t.human}</strong>
          <small>{t.humanFull}</small>
        </span>
        <span className="player-color">{t.black}</span>
      </div>
      <div className="player-divider">
        <span>VS</span>
      </div>
      <div
        className={`player-row ${!isFinished && game.turn === game.aiColor ? "is-active" : ""}`}
      >
        <span className="player-stone player-stone--white" />
        <span className="player-copy">
          <strong title={game.aiModelId ?? t.ai}>
            {game.aiModelId ?? t.ai}
          </strong>
          <small>{t.aiFull}</small>
        </span>
        <span className="player-color">{t.white}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <section className="tools-panel" id="tools" aria-labelledby="tools-title">
      <div className="tools-panel__head">
        ...
        <span
          className={`tool-connection ${webmcpStatus === "available" ? "is-live" : "is-muted"}`}
        >
          <span className="status-dot" />
          {webmcpStatus === "available" ? t.connected : t.offline}
        </span>
      </div>
      ...
      <div className="tool-list">
        {tools.map((tool) => (
          <div
            className={`tool-row ${lastToolCall === tool.code ? "is-called" : ""}`}
            key={tool.code}
          >
            ...
            {lastToolCall === tool.code && (
              <span className="tool-row__check">
                <Icon name="check" />
              </span>
            )}
          </div>
...
function MoveLog({
  ...
  return (
    <section className="move-log" aria-labelledby="move-log-title">
      <div className="move-log__head">
        <h2 id="move-log-title">{t.moveLog}</h2>
        <span>{moves.length.toString().padStart(2, "0")}</span>
      </div>
      {moves.length === 0 ? (
        <p className="empty-log">{t.emptyMoves}</p>
      ) : (
        <ol>
          {moves
            .slice(-5)
            .reverse()
            .map((move) => (
              <li key={`${move.number}-${move.actor}`}>
                <span className={`log-stone log-stone--${move.stone}`} />{" "}
                <span className="log-number">
                  {move.number.toString().padStart(2, "0")}
                </span>
                <span>
                  {move.pass
                    ? language === "zh"
                ...
                <small>{move.actor === "human" ? t.human : t.ai}</small>
              </li>
            ))}
        </ol>
      )}
...
const Board = memo(function Board({ board, interactive, lastMove, onMove, t }: BoardProps) {
  const size = board.length;
  const coordinates = "ABCDEFGHI";
  return (
    <div
      className={`board-surface ${interactive ? "is-interactive" : "is-static"}`}
    >
      <div
          ...
          row.map((cell, x) => {
            ...
            return (
              <button
                key={`${x}-${y}`}
                type="button"
                className={`intersection ${cell ? `has-${cell}` : ""} ${isLast ? "is-last" : ""}`}
                data-board-x={x}
                data-board-y={y}
                onClick={() => onMove?.({ x, y })}
...
function Icon({ name }: { name: IconName }) {
  return (
    ...
    >
      ...
      {name === "pass" && (
        <>
          <path d="M5 9c3-3 8-3 11 0" />
          <path d="m14 6 3 3-3 3" />
          <path d="M19 15c-3 3-8 3-11 0" />
      ...
      {name === "state" && (
        <>
          ...
          <path d="M13 6h6M13 10h6M5 16h14M5 20h9" />
        </>
      )}
      {name === "move" && (
        <>
